# 支付宝记账本支出看板 V21 - 标签/分类退款回冲版

## V21 更新

- 标签支出：退款会优先匹配回原始消费记录，同一标签下支出 300、退款 300，净花费显示 0。
- 分类支出：退款也会匹配回原始消费分类，不再简单显示为“退款抵扣”分类。
- 匹配规则：优先按相同金额、退款前的消费、相同账户/来源、备注相似度、日期接近程度匹配。
- 未能匹配到原消费的退款，才会保留为“退款抵扣”。
- 保留 V20 的标签支出 / 分类支出位置和总净支出金额显示。
...
支付宝记账本中，退款通常记录为：`分类 = 退款` 且 `收支类型 = 不计收支`。V20 开始，总支出卡片改为“净支出”：
...
- 标签支出和分类支出位置已互换，标签支出在前。
- 分类支出和标签支出标题右侧都显示净支出总金额。
- 分项金额使用净支出口径：支出为正，退款抵扣为负，分项合计与上方净支出卡片保持一致。
